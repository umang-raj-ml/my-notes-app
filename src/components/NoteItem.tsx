...
export default function NoteItem({note, onDelete, onEdit, onPin}: NoteItemProps) {
    return (
        <div className={`rounded shadow p-4 bg-gray-50 ${note.pinned ? 'border-l-4 border-yellow-400' : ''} `}>
            <div className="flex justify-between items-center mb-2">
                <h3 className="font-bold text-lg">{note.title}</h3>
                <button
                onClick={onPin}
                title={note.pinned ? 'Unpin' : 'Pin'}
                className={`ml-2 p-1 rounded ${note.pinned ? 'bg-yellow-300' : 'bg-gray-200 hover:bg-yellow-200'}`}
                >
